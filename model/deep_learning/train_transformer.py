"""
Transformer Model Training for EEG Migraine Severity Prediction
Multi-Head Attention Transformer for Long-Range Dependencies
...
def train_transformer_model(X_sequences, y_sequences, sequence_length, epochs=100, batch_size=128):
    """Train Transformer model"""
    print("\n" + "="*70)
    print("STEP 2: TRAINING TRANSFORMER MODEL")
    print("="*70)
    ...
    transformer_model.model.compile(optimizer=optimizer, loss='mse', metrics=['mae'])
    ...
    history = transformer_model.model.fit(
        X_train, y_train,
    ...
    # Evaluate model
    y_pred = transformer_model.model.predict(X_test, verbose=0)
    ...
    return transformer_model, history, {'r2': r2, 'rmse': rmse, 'mae': mae, 'training_time': training_time}

def save_model(model, processor, filepath_prefix='eeg_transformer'):
...
def main():
    ...
    print("="*70)
    print("\nMulti-Head Attention Transformer for EEG Migraine Severity Prediction")
    ...
    # Train model
    model, history, results = train_transformer_model(
