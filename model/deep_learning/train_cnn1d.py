"""
CNN1D Model Training for EEG Migraine Severity Prediction
...
def train_cnn1d_model(X_sequences, y_sequences, sequence_length, epochs=100, batch_size=128):
    """Train CNN1D model"""
    print("\n" + "="*70)
    print("STEP 2: TRAINING CNN1D MODEL")
    print("="*70)
    ...
    print("="*60)
    print("Architecture: Conv1D → BatchNorm → Pooling → Dense")
    print("Expected: Good for spatial pattern recognition in time-series")
    
    cnn1d_model = CNN1D_Model(sequence_shape, num_features)
    
    # Compile model
    optimizer = optimizers.Adam(learning_rate=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-7)
    cnn1d_model.model.compile(optimizer=optimizer, loss='mse', metrics=['mae'])
    ...
    # Evaluate model
    y_pred = cnn1d_model.model.predict(X_test, verbose=0)
    ...
    return cnn1d_model, history, {'r2': r2, 'rmse': rmse, 'mae': mae, 'training_time': training_time}

def save_model(model, processor, filepath_prefix='eeg_cnn1d'):
...
def main():
    print("="*70)
    print("CNN1D MODEL TRAINING")
    print("="*70)
    print("\n1D Convolutional Neural Network for EEG Migraine Severity Prediction")
    ...
    # Train model
    model, history, results = train_cnn1d_model(
